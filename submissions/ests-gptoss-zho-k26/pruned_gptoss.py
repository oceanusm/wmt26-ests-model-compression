    ...
    def __init__(
        ...
        *,
        ...
        if self.parallel_config.enable_expert_parallel:
            raise ValueError("Pruned GPT-OSS variable-expert models do not support expert parallel.")
        ...
        for name, weight in weights:
            ...
            if any(
                ...
                local_num_experts = int(
                    self.config.num_local_experts_per_layer[layer_idx]
                )
                checkpoint_num_experts = int(weight.shape[0])
                if checkpoint_num_experts != local_num_experts:
                    raise ValueError(
                        "Pruned GPT-OSS MXFP4 expert-axis mismatch for "
                        f"{name}: checkpoint={checkpoint_num_experts}, "
                        f"config_layer_{layer_idx}={local_num_experts}"
                    )
                param = params_dict.get(name)
                if param is not None and int(param.shape[0]) != local_num_experts:
                    raise ValueError(
                        "Pruned GPT-OSS MXFP4 allocated expert-axis mismatch for "
                        f"{name}: parameter={int(param.shape[0])}, "
                        f"config_layer_{layer_idx}={local_num_experts}"
                    )
            ...
            if ".w13_weight_scale" in name:
                ...
                assert local_num_experts is not None
                weight = weight.view(
                    local_num_experts, 2 * intermediate_size, -1
                ).contiguous()
                ...
                assert local_num_experts is not None
                weight = weight.view(
                    local_num_experts, -1, intermediate_size // 2
                ).contiguous()
                if use_ep:
                    narrow_weight = weight[ep_rank_start:ep_rank_end, ...]
                else:
